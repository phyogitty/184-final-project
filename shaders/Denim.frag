...
void main() {
    ...
    float dU = (h(v_uv + vec2(1/u_texture_2_size[0], 0.0)) - h(v_uv)) * u_height_scaling * u_normal_scaling;
    float dV = (h(v_uv + vec2(0.0, 1/u_texture_2_size[1])) - h(v_uv)) * u_height_scaling * u_normal_scaling;
    ...
    float p = 20.0;						        // jean
    float k_a = 0.5, k_d = 0.5, k_s = 0.05;	    // jean
